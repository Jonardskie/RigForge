...
export interface ComponentSeed {
  ...
  sku: string;
  name: string;
  category: 'CPU' | 'Motherboard' | 'GPU' | 'RAM' | 'Cooler' | 'Case' | 'PSU' | 'Display';
  socket?: string;
  watts?: number;
  lengthMm?: number;
  maxGpuLength?: number;
...
export interface RuleSeed {
  ...
  storeId: string;
  name: string;
  sourceCategory: 'CPU' | 'Motherboard' | 'GPU' | 'RAM' | 'Cooler' | 'Case' | 'PSU' | 'Display';
  targetCategory: 'CPU' | 'Motherboard' | 'GPU' | 'RAM' | 'Cooler' | 'Case' | 'PSU' | 'Display';
...
export interface BuildSeed {
  ...
  status: 'PENDING_VERIFICATION' | 'APPROVED' | 'IN_ASSEMBLY' | 'COMPLETED';
  createdAt?: Date;
}
...
export interface ActivityLogSeed {
  id: string;
  storeId: string;
  actionType: 'STOCK_RESTOCK' | 'RULE_CREATED' | 'RULE_TOGGLED' | 'BUILD_VERIFIED' | 'COMPONENT_ADDED' | 'PRICE_UPDATED';
...
export const initialComponents: ComponentSeed[] = [
  // CPUs
  {
    id: 'cpu-7800x3d',
...
export const initialRules: RuleSeed[] = [
  {
...
export const initialBuilds: BuildSeed[] = [
  {
    id: 'build-rf-9082',
    storeId: initialStore.id,
    orderNumber: '#RF-9082',
...
export const initialActivityLogs: ActivityLogSeed[] = [
  {
    id: 'log-001',
    storeId: initialStore.id,
